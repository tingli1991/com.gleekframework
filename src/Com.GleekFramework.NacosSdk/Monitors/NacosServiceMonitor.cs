...
namespace Com.GleekFramework.NacosSdk
{
    ...
    static class NacosServiceMonitor
    {
        ...
        {
            timer?.Dispose();//释放旧定时器
            timer = new Timer(async e =>
            {
                try
                {
                    await nacosConf.SendServiceInstanceBeatAsync(serverName);
                }
                catch (Exception ex)
                {
                    NLogProvider.Error($"【Nacos心跳检查回调】发生未经处理的异常：{ex}");
                }
            }, null, TimeSpan.Zero, TimeSpan.FromMilliseconds(nacosConf.ListenInterval));
        }
        ...
        public static async Task RegisterInstanceAsync(this NacosSettings nacosConf, string serverName)
        {
            try
            {
                ...
                var token = await baseUrl.GetAuthTokenAsync(nacosConf.UserName, nacosConf.Password);//获取授权的token
