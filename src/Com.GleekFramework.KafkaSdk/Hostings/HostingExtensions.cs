...
namespace Com.GleekFramework.KafkaSdk
{
    ...
    public static partial class KafkaHostingExtensions
    {
        ...
        public static IHost SubscribeKafka(this IHost host, Func<IConfiguration, KafkaConsumerOptions> callback)
        {
            ...
            host.RegisterApplicationStarted(() => { _ = Subscribe(options); })
        ...
        private static async Task Subscribe(KafkaConsumerOptions options)
        {
            try
            {
                if (options == null)
                ...
                foreach (var hostOptions in options.HostOptions)
                {
                    ...
                    await KafkaTopicProvider.CreateTopicAsync(hostOptions.Host, hostOptions.Topics);//初始化主题
